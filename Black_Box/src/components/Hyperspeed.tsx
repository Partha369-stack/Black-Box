...
const deepUniforms = {
  uFreq: { value: new THREE.Vector4(4, 8, 2, 1) },
  uAmp: { value: new THREE.Vector4(10, 20, 5, 2) },
...
class App {
  ...
  loadAssets(): Promise<void> {
    // No assets to load for this simplified version
...
const Hyperspeed: FC<HyperspeedProps> = ({ effectOptions = {} }) => {
  ...
  useEffect(() => {
    console.log('🚀 Hyperspeed effect starting...');
    ...
    if (appRef.current) {
      console.log('♻️ Disposing previous Hyperspeed instance');
    ...
    if (!container) {
      console.error('❌ No container found for Hyperspeed');
      return;
    }
    ...
    // Wait for container to be properly sized before initializing WebGL
    initTimeoutRef.current = setTimeout(() => {
      if (!container || !hyperspeed.current) {
        console.error('❌ Container disappeared during timeout');
        return;
      }
      ...
      if (rect.width === 0 || rect.height === 0) {
        console.error('🔧 Hyperspeed container has zero dimensions, forcing size...');
        ...
        setTimeout(() => {
          ...
          if (newRect.width === 0 || newRect.height === 0) {
            console.error('❌ Still zero dimensions after resize attempt');
            return;
          }
          initializeApp();
        }, 100);
      ...
      function initializeApp() {
        console.log('🎬 Initializing Hyperspeed App...');
        
        const options = { ...mergedOptions };
        if (typeof options.distortion === "string") {
          console.log('🌀 Using distortion:', options.distortion);
        ...
        try {
          ...
          myApp.loadAssets().then(() => {
            ...
            if (appRef.current === myApp) { // Check if still the current app
              myApp.init();
              console.log('🎉 Hyperspeed initialized successfully!');
            } else {
              console.warn('⚠️ App reference changed, skipping init');
            }
          }).catch((error) => {
          ...
          console.warn('🔄 Attempting to reinitialize Hyperspeed after failed attempt...');
          setTimeout(() => {
            if (!container || !hyperspeed.current) return;
            try {
              const retryApp = new App(container, options);
              appRef.current = retryApp;
              retryApp.loadAssets().then(() => {
                if (appRef.current === retryApp) {
                  retryApp.init();
                  console.log('🎉 Hyperspeed retry successful!');
                }
              }).catch((retryError) => {
  ...
  return (
    <div 
      ...
      style={{
        width: '100%',
        height: '100%',
        minHeight: '400px', // Ensure minimum dimensions
        position: 'relative'
      }}
    />
  );
